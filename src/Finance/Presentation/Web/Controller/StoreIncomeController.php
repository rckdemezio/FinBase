...
final class StoreIncomeController
{
    ...
    public function __invoke(Request $request): Response
    {
        ...
        try {
            $this->recordIncome->execute(
                $accountId,
                new Money($amount, $account->balance()->currencyCode()),
                $description,
                $occurredAt,
            );
        } catch (InvalidAccountAmountException | InvalidTransactionAmountException $exception) {
    ...
    private function renderForm(Account $account, array $errors, array $old): HtmlResponse
    {
        return new HtmlResponse($this->view->render('layouts/app', [
            'title' => 'Nova renda',
            'content' => $this->view->render('transactions/income-create', compact('account', 'errors', 'old')),
        ]), 422);
    }
}
